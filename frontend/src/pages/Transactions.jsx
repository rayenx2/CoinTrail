...
import { Search, Plus, Trash2, Calendar, FileDown } from 'lucide-react';
...
export default function Transactions() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter(t =>
      t.description?.toLowerCase().includes(q) || t.categoryName?.toLowerCase().includes(q)
    );
  }, [transactions, search]);
  ...
  const fetchTransactions = async (targetPage = page) => {
    try {
      const { data } = await transactionService.getAll(targetPage, PAGE_SIZE);
      setTransactions(data.content || []);
      setPageInfo({ totalPages: data.totalPages, first: data.first, last: data.last });
      setPage(data.number);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    >
      ...
      <div className="flex justify-between items-center px-4 pt-4">
        <p className="text-muted text-xs font-black uppercase tracking-widest">
          Page {page + 1} of {Math.max(pageInfo.totalPages, 1)} · {filteredTransactions.length} shown
        </p>
        <div className="flex gap-2">
           <button
             onClick={() => fetchTransactions(page - 1)}
             disabled={pageInfo.first}
             className="px-6 py-2 rounded-xl bg-card border border-border text-muted font-black text-[10px] uppercase tracking-widest disabled:opacity-30 active:scale-95 transition-all"
           >
             Prev
           </button>
           <button
             ...
             className="px-6 py-2 rounded-xl bg-primary text-primary-foreground font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primary/20 active:scale-95 transition-all disabled:opacity-30"
           >
             Next
           </button>
        </div>
